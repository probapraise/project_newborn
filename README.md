...
python3 -m lifeops.cli init-db
python3 -m lifeops.server --host 127.0.0.1 --port 8765
```

간단한 검증은 아래 스크립트를 사용한다.

```bash
./scripts/test.sh
```

Windows bridge는 WSL core가 켜진 뒤 별도 PowerShell에서 실행한다.

```powershell
pwsh -NoProfile -ExecutionPolicy Bypass -File .\windows_bridge\Run-ActivityBridge.ps1
```
